      ...
      <div class="navbar-menu" :class="{ open: isMenuOpen }">
        <div class="nav-links">
          ...
          <RouterLink to="/teacher" class="nav-link" @click="closeMenu">
            ...
            <span>{{ t('nav.becomeTeacher') }}</span>
          </RouterLink>
        </div>

        <div class="nav-actions">
          <!-- Theme Toggle -->
          <button @click="toggleTheme" class="icon-btn" :title="theme === 'light' ? 'Switch to dark mode' : 'Switch to light mode'">
            <svg v-if="theme === 'light'" width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const languages = [
  ...
  { code: 'ru' as Language, label: 'РУ', name: 'Русский' },
  { code: 'kk' as Language, label: 'ҚЗ', name: 'Қазақша' }
]

// Make translation reactive by depending on currentLanguage
const t = (key: string) => {
  // Access currentLanguage.value to create reactive dependency
