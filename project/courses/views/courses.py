...
class CourseDetailView(APIView):
    ...
    def get(self, request, slug):
        try:
            ...
            # Serialize the course without videos first
            ...
            if not has_access:
                ...
                preview_videos = list(data.get('videos', []))
                data['videos'] = [v for v in preview_videos if v.get('is_preview', False)]
...
class PurchasedCoursesView(APIView):
    ...
    def get(self, request):
        try:
            purchased = UserPayment.objects.filter(app_user=request.user, payment_bool=True).values_list('course', flat=True)
            courses = Course.objects.filter(id__in=purchased)
            serializer = CourseSerializer(courses, many=True, context={'request': request})
            return Response(serializer.data, status=status.HTTP_200_OK)
        except Exception as e:
            return Response({'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        ...
        try:
            ...
            if not has_access:
                return Response({
                    ...
                    'total_tests': 0,
                    'passed_tests': 0,
                    'can_take_final': False,
                    'can_generate_certificate': False
                }, status=status.HTTP_200_OK)
            
            # Get all videos in course
            videos = course.video_set.all()
            total_tests = videos.count()
            
            # Get test results for this user
            test_results = TestResult.objects.filter(user=request.user, video__course=course)
            completed_tests = test_results.count()
            
            # Get passed tests (score / total_questions >= 0.7)
            passed_tests = sum(1 for tr in test_results if (tr.score / tr.total_questions) >= 0.7)
            
            # User can take final exam if all tests are passed
            can_take_final = passed_tests == total_tests and total_tests > 0
            
            # Get user course to check if completed
            user_course, created = UserCourse.objects.get_or_create(user=request.user, course=course)
            completed = user_course.completed
            can_generate_certificate = completed
